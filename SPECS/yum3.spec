...
%define real_name yum
%define basever 3

Summary: RPM installer/updater
Name: yum3
Version: 3.2.22
Release: 20.0.1.ius%{?dist}
License: GPLv2+
Group: System Environment/Base
Source0: http://linux.duke.edu/projects/yum/download/3.2/%{real_name}-%{version}.tar.gz
...
Patch200: yum-3.2.22-sourcerpm_compare.patch
...
Provides: %{name}-updatesd = %{version}-%{release}
Conflicts: %{real_name} < %{basever}
...
%changelog
* Fri Oct 16 2009 BJ Dierkes <wdierkes@rackspace.com> - 3.2.22-20.0.1.ius
- Rebuilding for IUS only to provide the fix for this issue (not bug)
  http://yum.baseurl.org/ticket/296
- IUS will only maintain yum3 as an optional workaround until the
  capabilities of handling alternate name packages exists (php52 vs php).
- Added Patch200: yum-3.2.22-sourcerpm_compare.patch
